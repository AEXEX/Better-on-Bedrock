import { world, system, BlockTypes, BlockStates } from "@minecraft/server";
...
export const toolTip = (player) => {
    ...
    if (
            ...
            player.onScreenDisplay.setActionBar(
                {
                    rawtext: [
                        ...
                        {
                            text: `${entityInfo?.entityInfo ? (`\n§7§3${entityInfo.entityInfo}§r`) : ""}`,
                        },
                        { text: `\n§9§o${addonId}` },
                    ],
                },
        ...
        const capitalizedWords = namePart.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1));
        ...
        player.onScreenDisplay.setActionBar(
            {
                rawtext: [
                    ...
                    {
                        text: `${compostLevel ? (`\n§7Compost Level: ${compostLevel}/8`) : ""}`,
                    },
                    {
                        text: `${tool?.tool ? (`\n§7Correct Tool: §3${tool.tool}§r\n§7Can Harvest: ${tool.farmable ? "§aYes" : "§cNo"}`) : ""}`,
                    },
                    {
                        text: `${blockInfo?.blockInfo ? (`\n§7§3${blockInfo.blockInfo}§r`) : ""}`,
                    },
...
const getEntityInfo = (entity) => {
    if (entity.typeId.includes("corrupted")) {
        return {
            entityInfo: "§eCure Items: §gGolden Apple"
        };
    };
    if (entity.typeId.includes("cured")) {
        return {
            entityInfo: "§7Info: §eCan Trade"
        };
    };
    if (entity.typeId.includes("raccoon")) {
        return {
            entityInfo: "§7Info: §eCan dig treasure at random times"
        };
    };
    ;
};
const getBlockInfo = (block) => {
    if (block.typeId.includes("enchant")) {
        return {
            blockInfo: "§cInfo: §eHold a tool to use."
        };
    };
    ;
};
